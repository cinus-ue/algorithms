...
func TestInsertSameKeyTwice(t *testing.T) {
	...
	err = tree.Put(key, append(value, []byte("world1")...))
	if err == nil {
...
func TestMultiInsertSingleDelete(t *testing.T) {
	...
	err = tree.Put(key+1, append(value, []byte("world1")...))
	if err != nil {
		t.Errorf("%s", err)
	}
	err = tree.Put(key+2, append(value, []byte("world2")...))
	if err != nil {
		t.Errorf("%s", err)
	}
	err = tree.Put(key+3, append(value, []byte("world3")...))
	if err != nil {
		t.Errorf("%s", err)
	}
	err = tree.Put(key+4, append(value, []byte("world4")...))
	if err != nil {
		t.Errorf("%s", err)
	}
